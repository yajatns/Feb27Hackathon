      ...
      <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--border)] overflow-hidden">
        <div className="px-5 py-3 border-b border-[var(--border)] flex items-center justify-between">
          ...
          <span className="text-xs text-[var(--text-secondary)]">
            {new Date(hire.created_at).toLocaleString()}
          </span>
        </div>

        {hire.tasks.length === 0 ? (
          <div className="p-5 text-sm text-[var(--text-secondary)]">No agent tasks recorded.</div>
        ) : (
          <div className="divide-y divide-[var(--border)]">
            {hire.tasks.map((t, i) => {
              ...
              return (
                <details key={t.id} className="group" open={true}>
                  ...
                  <div className="px-5 pb-4 pl-14">
                    ...
                    {t.input_data && (
                      <div className="mb-2">
                        <span className="text-[10px] text-[var(--text-secondary)] uppercase">Task:</span>
                        <div className="text-xs text-[var(--text-secondary)] mt-0.5">
                          {(() => { try { const d = JSON.parse(t.input_data); return d.task || t.input_data; } catch { return t.input_data; }})()}
                        </div>
                      </div>
                    )}
                    {/* Agent reasoning */}
